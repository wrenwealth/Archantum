...
- You'll receive automatic alerts for your watched markets"""
    ...
    async def cmd_getid(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            # First try to fetch directly from Gamma API by event slug
            async with GammaClient() as client:
                api_markets = await client.get_markets_by_event_slug(slug)

                if api_markets:
                    # Save to database for future lookups
                    await self.db.upsert_markets(api_markets)

                    text = f"<b>Markets for '{slug}':</b>\n\n"
                    for m in api_markets[:10]:
            ...
            markets = await self.db.search_markets(search_terms, limit=5)
            if markets:
